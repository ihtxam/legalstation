...
If Docker bridge networking is broken in a nested VM, use the host-network overlay:

```bash
docker compose -f docker-compose.yml -f docker-compose.cloudtest.yml up -d --build
```
