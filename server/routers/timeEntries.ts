...
export const timeEntriesRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return getTimeEntriesByFirm(member.firmId, {
        lawyerId: input?.mineOnly === false && member.firmRole === "admin" ? undefined : ctx.user.id,
        caseId: input?.caseId,
        status: input?.status,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (entry.lawyerId !== ctx.user.id && member.firmRole !== "admin") {
  ...
  submitMany: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const id of input.ids) {
        ...
        if (!entry) continue;
        if (entry.lawyerId !== ctx.user.id && member.firmRole !== "admin") continue;
